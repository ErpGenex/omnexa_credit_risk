...
def policy_query_conditions(user=None):
	...
	return f"(`tabCredit Risk Policy Version`.maker = {esc} or `tabCredit Risk Policy Version`.checker = {esc} or `tabCredit Risk Policy Version`.rejector = {esc})"
...
def snapshot_query_conditions(user=None):
	...
	if _is_privileged(user):
		return ""
	esc = frappe.db.escape(user)
	return f"`tabCredit Risk Audit Snapshot`.actor = {esc}"
