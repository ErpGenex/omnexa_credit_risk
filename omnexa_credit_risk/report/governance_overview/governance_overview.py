...
def execute(filters=None):
	...
	pol_total = frappe.db.count("Credit Risk Policy Version")
	pending = frappe.db.count("Credit Risk Policy Version", {{"status": "PENDING_APPROVAL"}})
	approved = frappe.db.count("Credit Risk Policy Version", {{"status": "APPROVED"}})
	rejected = frappe.db.count("Credit Risk Policy Version", {{"status": "REJECTED"}})
	snaps = frappe.db.count("Credit Risk Audit Snapshot")
